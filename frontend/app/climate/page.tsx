...
function StatCard({ icon: Icon, label, value, unit, sub, color }: {
  icon: any; label: string; value: number; unit: string; sub?: string; color: string;
}) {
  return (
    <Card>
...
export default function ClimatePage() {
  const [location, setLocation] = useState("balige");
  const [data, setData] = useState<ClimateStats | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const fetchClimate = async (loc: string) => {
    setLoading(true);
    setError(null);
    try {
  ...
  useEffect(() => { fetchClimate(location); }, [location]);

  const barData = data ? [
    { name: "Suhu Max Avg", value: data.avg_temp_max, fill: "#f97316" },
    { name: "Hujan Avg",    value: data.avg_precipitation, fill: "#0ea5e9" },
    { name: "Suhu Max",     value: data.max_temp, fill: "#ef4444" },
    { name: "Hujan Max",    value: data.max_precipitation, fill: "#6366f1" },
  ] : [];
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={barData} layout="vertical">
                  ...
                  <YAxis dataKey="name" type="category" tick={{ fontSize: 11 }} stroke="var(--muted-foreground)" width={110} />
          ...
          <Card>
            <CardContent className="p-5">
              <p className="text-sm text-muted-foreground text-center">
                Data iklim historis <span className="font-semibold text-foreground">{locationLabel}</span> periode{" "}
